...
CARGO_TARGET_X86_64_UNKNOWN_LINUX_GNU_LINKER="clang" RUSTFLAGS="-Clink-arg=-fuse-ld=lld -Clink-arg=-Wl,--icf=all -C target-cpu=$TARGET_CPU -Zlocation-detail=none -Zshare-generics=y -Zthreads=0" cargo +nightly build -Z build-std=core,std,panic_abort,alloc,proc_macro -Z build-std-features=panic-unwind ${COMPRESSION:""} --target x86_64-unknown-linux-gnu --release
